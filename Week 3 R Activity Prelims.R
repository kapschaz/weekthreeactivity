...
numrows <- nrow(KapambweTitanic)

numcols <- ncol(KapambweTitanic)

columnnames <- colnames(KapambweTitanic)

columnclasses <- sapply(KapambweTitanic, class)
...
Sexcolumn <- KapambweTitanic$Sex 
Agecolumn <- KapambweTitanic$Age
...
newdataframe <- data.frame(Sex = Sexcolumn, Age = Agecolumn)
...
subsetdata <- KapambweTitanic[KapambweTitanic$Freq > 25, ]
...
#ggplot(Yourdataframename, aes(columnname, columnname)) +
 # geom_col()
ggplot(KapambweTitanic, aes(x = Survived)) +
  geom_bar()
